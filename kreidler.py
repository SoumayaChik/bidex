...
def get_battery_attributes(battery_column, battery_capacity_column):
     
    battery_capacity_pattern =r', (\d+) Wh'
    match = re.search(battery_capacity_pattern,  battery_column)
    if match:
        extracted_battery_capacity = match.group(0)
        clean_battery = battery_column.replace(extracted_battery_capacity, '')
        if battery_capacity_column =='':
            clean_battery_capacity =extracted_battery_capacity.replace(', ','')
        else :
            clean_battery_capacity =battery_capacity_column.replace(',00','')+' Wh'
    else:
        ...
        if battery_capacity_column =='':
            ...
            clean_battery_capacity =battery_capacity_column.replace(',00','')+' Wh'
    if battery_column =='':
        clean_battery_position =''
    else :
        clean_battery_position = battery_column.split(',')[0].split()[-1].strip()
        clean_battery = clean_battery.replace(clean_battery_position,'')
    return pd.Series([clean_battery, clean_battery_capacity, clean_battery_position])



df[['battery', 'battery_capacity', 'battery_position']] =df.apply(lambda x: get_battery_attributes(x['battery'], x['battery_capacity']), axis=1)


# #### Size


def get_size_attributes(size_column):
    size_column = size_column.replace(':','').replace('/','')
    splitted_size_column =size_column.split() 
    clean_frame_shape = splitted_size_column[0]
    clean_wheel_size = splitted_size_column[1].replace(',','.')+'"'
    clean_frame_size = ' '.join(splitted_size_column[-2::])
    return pd.Series([clean_frame_shape, clean_wheel_size, clean_frame_size])


df[['frame_shape', 'wheel_size', 'frame_size']] =df.apply(lambda x: get_size_attributes(x['size']), axis=1)


# #### Model Name


def clean_model_name(model_name_column, battery_capacity_column, wheel_size):
    model_name_column =model_name_column.replace(',','.').replace('er','"').replace(' '+wheel_size,'')
    if 'Wh' in model_name_column:
        clean_model_name = model_name_column.replace(battery_capacity_column.replace(' Wh','')+'Wh','')
    else:
        clean_model_name= model_name_column
    return  pd.Series([clean_model_name.strip()])


df['model_name'] =df.apply(lambda x: clean_model_name(x['model_name'], x['battery_capacity'], x['wheel_size']), axis=1)


# #### Engine


def get_engine_attributes(engine_column):
    if engine_column =='':
        clean_engine_power =''
        clean_engine =''
    else:
        splitted_engine_power = engine_column.replace('Watt','').split(',')
        clean_engine_power =splitted_engine_power[-1].strip()
        clean_engine = ','.join(splitted_engine_power[:-1])
    clean_engine_torque =''
    clean_engine_speed =''
    return  pd.Series([clean_engine, clean_engine_power, clean_engine_torque, clean_engine_speed])


df[['engine', 'engine_power', 'engine_torque','engine_speed']] =df.apply(lambda x: get_engine_attributes(x['engine']), axis=1)
...
def get_rear_derailleur_attributes(rear_derailleur_column):
    gear_count_pattern  = re.compile(r"\b, (\d+)-fach\b|\b (\d+)-Gang\b")
    gear_count_match = re.search(gear_count_pattern,  rear_derailleur_column)
    if gear_count_match:
        if gear_count_match.group(1):
            extracted_gear_count = gear_count_match.group(1)  
            clean_gear_count = extracted_gear_count
        elif gear_count_match.group(2):
            extracted_gear_count = gear_count_match.group(2)
            clean_gear_count = extracted_gear_count
        clean_rear_deraillleur = rear_derailleur_column.replace(extracted_gear_count,'').replace('-fach','').replace(', ','').replace('-Gang','')
    else:
        clean_gear_count = ''
        clean_rear_deraillleur = rear_derailleur_column
    return  pd.Series([clean_gear_count, clean_rear_deraillleur])


df[[ 'gear_count','rear_derailleur']] =df.apply(lambda x: get_rear_derailleur_attributes(x['rear_derailleur']), axis=1)
...
df['rrp'] = df['price'].apply(lambda x: x.replace(',90', ''))
...
df[cols].to_csv('kreidler.csv', index = 0)

import boto3
s3_resource = boto3.client('s3')
s3_resource.upload_file('/usr/share/tomcat8/kreidler.csv', 'bi-rebike', 'Bidex/kreidler.csv')