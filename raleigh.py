...
# ##### Model_name and color are clean
...
def get_battery_attributes(battery_column, battery_capacity_column):
    battery_capacity_column = battery_capacity_column.replace(',00','').strip()
    if battery_column.isnumeric():
        clean_battery =''
        if (len(battery_capacity_column) ==2 or battery_capacity_column ==''):
            clean_battery_capacity =battery_column +' Wh'    
        else:
...
def get_size_attributes(size_column, model_year):
    size_column = size_column.replace(' Zoll','"').replace(',', '.')
    if model_year =='2020':
        splitted_size_column = size_column.split('-')
        clean_frame_size =splitted_size_column[0].strip()
        clean_frame_shape = splitted_size_column[1].strip()
        clean_wheel_size = splitted_size_column[2].strip()
    else:
        splitted_size_column = size_column.split(' ')
        clean_frame_size =' '.join(splitted_size_column[2::]).strip()
        clean_frame_shape = splitted_size_column[0].strip()
        clean_wheel_size = splitted_size_column[1].strip()
    return pd.Series([clean_frame_shape, clean_wheel_size, clean_frame_size])


df[['frame_shape', 'wheel_size', 'frame_size']] =df.apply(lambda x: get_size_attributes(x['size'], x['model_year']), axis=1)
...
def get_engine_attributes(engine_column):
    if engine_column =='':
        ...
        splitted_engine_column = engine_column.split('/')
        clean_engine =splitted_engine_column[0].strip()
        clean_engine_power =splitted_engine_column[1].replace(' W','').strip()
        clean_engine_torque =splitted_engine_column[2].strip()
    clean_engine_speed=''
    return  pd.Series([clean_engine, clean_engine_power, clean_engine_torque, clean_engine_speed])


df[['clean_engine', 'engine_power', 'engine_torque', 'engine_speed']] =df.apply(lambda x: get_engine_attributes(x['engine']), axis=1)
df['engine'] = df['clean_engine']

# #### Rear Derailleur


def get_rear_derailleur_attributes(rear_derailleur_column):
    gear_count_position = rear_derailleur_column.find('-G')
    if gear_count_position !=-1:
        clean_gear_count = rear_derailleur_column[0:gear_count_position]
        clean_rear_derailleur = rear_derailleur_column[gear_count_position+3:]
    else:
        clean_gear_count = ''
        clean_rear_derailleur =rear_derailleur_column
    return  pd.Series([clean_gear_count, clean_rear_derailleur])
...
df['rrp'] = df['price'].apply(lambda x: x.replace(',99', ''))
...
df[cols].to_csv('raleigh.csv', index = 0)


import boto3
s3_resource = boto3.client('s3')
s3_resource.upload_file('/usr/share/tomcat8/raleigh.csv', 'bi-rebike', 'Bidex/raleigh.csv')