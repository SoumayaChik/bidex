...
# ##### Color and Model Name are clean
...
def clean_battery_capacity (battery_capacity_column):
    clean_battery_capacity = battery_capacity_column.replace(',00', ' Wh')
    return pd.Series([clean_battery_capacity])


df['battery_capacity'] =df.apply(lambda x: clean_battery_capacity( x['battery_capacity']), axis=1)


def get_size_attributes(size_column):
    size_column = size_column.replace(',5','.5').replace('Zoll','"').replace(' "','"').replace(' (','/').replace(')','').replace(' ',',').replace(',,',',')
    splitted_size =size_column.split(',')
    clean_frame_shape = splitted_size[0]
    clean_frame_size =splitted_size[1]
    try:
        clean_wheel_size =splitted_size[2]
        try:
            clean_wheel_size = splitted_size[2].split(r'/')[0]
            clean_frame_size =clean_frame_size +'/'+splitted_size[2].split('/')[1]
        except: 
            clean_wheel_size = splitted_size[2]
            clean_frame_size =clean_frame_size
    except :
        clean_wheel_size=''
...
def get_engine_attributes(engine_column):
    print(engine_column)
    splitted_engine =engine_column.split('/')
    clean_engine = splitted_engine[0]
    clean_engine_power =splitted_engine[1].replace('W','').strip()
    try:
        clean_engine_torque=' '.join(splitted_engine[2].strip().split()[::2])
        clean_engine = clean_engine +splitted_engine[2].strip().split()[2]
    except:
        clean_engine_torque=splitted_engine[2]
    clean_engine_speed =''
...
def get_engine_attributes(engine_column):
    if engine_column !='':
        splitted_engine =engine_column.split('/')
        clean_engine = splitted_engine[0].strip()
        clean_engine_power =splitted_engine[1].replace('W','').strip()
        try:
            clean_engine_torque=' '.join(splitted_engine[2].strip().split()[:2])
            clean_engine = clean_engine +splitted_engine[2].strip().split()[2]
        except:
            clean_engine_torque=splitted_engine[2].strip()
        clean_engine_speed =''
    else :
        clean_engine =''
        clean_engine_power =''
...
def get_rear_derailleur_attributes(rear_derailleur_column):
    gear_count_pattern  = re.compile(r", (\d+)-speed|(\d+)-G")
    gear_count_match = re.search(gear_count_pattern,  rear_derailleur_column)
    if gear_count_match:
        extracted_gear_count= gear_count_match.group(0)
        clean_gear_count = extracted_gear_count.replace('-speed','').replace(',','').replace('-G','').strip()
        clean_rear_deraillleur = rear_derailleur_column.replace(extracted_gear_count,'')
    else:
        clean_gear_count = ''
...
df[cols].to_csv('kalkhoff.csv', index = 0)


import boto3
s3_resource = boto3.client('s3')
s3_resource.upload_file('/usr/share/tomcat8/kalkhoff.csv', 'bi-rebike', 'Bidex/kalkhoff.csv')
